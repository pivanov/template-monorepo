  ...
  for (let i = 0; i < frameCount; i++) {
    ...
    for (const size of sizes) {
      ...
      const response = await fetch(
        browser.runtime.getURL('icon/icon-no-corners.png'),
      );
      ...
      ctx.lineDashOffset =
        (progress * -perimeter) / (2 * Math.PI) - dashLength / 2;
...
export const updateIcon = async (enabled: boolean | null): Promise<void> => {
  ...
  if (
    ...
    now - lastStateChangeTime < MIN_STATE_CHANGE_INTERVAL
  ) {
    return;
  }
